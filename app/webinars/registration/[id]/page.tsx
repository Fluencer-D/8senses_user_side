...
const Registration = () => {
  ...
  const [webinarDetails, setWebinarDetails] = useState<WebinarDetails | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!webinarId) return alert("Webinar ID not found!");
  
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/webinars/public/register/${webinarId}`, {
        method: "POST",
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-30 max-w-5xl mx-5  mt-10 ml-25 max-[1280px]:ml-5 max-[1280px]:mt-8 max-[1280px]:gap-6">
        ...
        {webinarDetails && (
          <div className="bg-white text-black h-70 p-4 rounded-lg w-[120%] shadow-lg border-[2px] border-[#1E437A] max-[1280px]:w-full max-[1280px]:p-3">
            <div className='flex gap-3 max-[1280px]:gap-2'>
              <p className="mt-4 text-[#456696] font-nav_link_font font-medium gap-2 flex text-lg rounded-2xl bg-[#245BA729] w-[30%] h-7 items-center max-[1280px]:text-base max-[1280px]:h-6 max-[1280px]:w-auto max-[1280px]:px-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="23" viewBox="0 0 24 25" fill="none" className="max-[1280px]:w-4 ml-2 max-[1280px]:h-4">
              ...
              </svg> {webinarDetails.date?.slice(0, 10)}
              </p>
              <p className="mt-4 text-[#456696] font-nav_link_font font-medium gap-2 flex text-lg rounded-2xl bg-[#245BA729] w-[20%] h-7 items-center max-[1280px]:text-base max-[1280px]:h-6 max-[1280px]:w-auto max-[1280px]:px-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none" className="max-[1280px]:w-4 max-[1280px]:h-4 ml-2">
                <path d="M12 2.5C17.523 2.5 22 6.977 22 12.5C22 18.023 17.523 22.5 12 22.5C6.477 22.5 2 18.023 2 12.5C2 6.977 6.477 2.5 12 2.5ZM12 6.5C11.7348 6.5 11.4804 6.60536 11.2929 6.79289C11.1054 6.98043 11 7.23478 11 7.5V12.5C11.0001 12.7652 11.1055 13.0195 11.293 13.207L14.293 16.207C14.4816 16.3892 14.7342 16.49 14.9964 16.4877C15.2586 16.4854 15.5094 16.3802 15.6948 16.1948C15.8802 16.0094 15.9854 15.7586 15.9877 15.4964C15.99 15.2342 15.8892 14.9816 15.707 14.793L13 12.086V7.5C13 7.23478 12.8946 6.98043 12.7071 6.79289C12.5196 6.60536 12.2652 6.5 12 6.5Z" fill="#245BA7"/>
              </svg> {webinarDetails.startTime}
              </p>
            </div>

            <h3 className="text-xl font-semibold mt-2 text-[#1E437A] max-[1280px]:text-lg">{webinarDetails.title}</h3>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 32 32" fill="none" className="max-[1280px]:w-6 max-[1280px]:h-6">
              <path d="M6.66667 26.6667C5.93333 26.6667 5.30578 26.4059 4.784 25.8841C4.26222 25.3623 4.00089 24.7343 4 24.0001V14.6667C4 13.0223 4.31689 11.4721 4.95067 10.0161C5.58444 8.56008 6.44533 7.28808 7.53333 6.20008C8.62133 5.11208 9.89378 4.25119 11.3507 3.61741C12.8076 2.98364 14.3573 2.66675 16 2.66675C17.6427 2.66675 19.1929 2.98364 20.6507 3.61741C22.1084 4.25119 23.3804 5.11208 24.4667 6.20008C25.5529 7.28808 26.4142 8.56053 27.0507 10.0174C27.6871 11.4743 28.0036 13.0241 28 14.6667V28.0001C28 28.7334 27.7391 29.3614 27.2173 29.8841C26.6956 30.4067 26.0676 30.6676 25.3333 30.6667H17.3333C16.9556 30.6667 16.6391 30.5387 16.384 30.2827C16.1289 30.0267 16.0009 29.7103 16 29.3334C15.9991 28.9565 16.1271 28.6401 16.384 28.3841C16.6409 28.1281 16.9573 28.0001 17.3333 28.0001H25.3333V26.6667H22.6667C21.9333 26.6667 21.3058 26.4059 20.784 25.8841C20.2622 25.3623 20.0009 24.7343 20 24.0001V18.6667C20 17.9334 20.2613 17.3059 20.784 16.7841C21.3067 16.2623 21.9342 16.001 22.6667 16.0001H25.3333V14.6667C25.3333 12.089 24.4222 9.88897 22.6 8.06675C20.7778 6.24453 18.5778 5.33341 16 5.33341C13.4222 5.33341 11.2222 6.24453 9.4 8.06675C7.57778 9.88897 6.66667 12.089 6.66667 14.6667V16.0001H9.33333C10.0667 16.0001 10.6947 16.2614 11.2173 16.7841C11.74 17.3067 12.0009 17.9343 12 18.6667V24.0001C12 24.7334 11.7391 25.3614 11.2173 25.8841C10.6956 26.4067 10.0676 26.6676 9.33333 26.6667H6.66667Z" fill="#456696"/>
            </svg>  Speaker: <span className='font-normal'>{webinarDetails.speaker}</span>
            </p>
