...
const services: Service[] = [
  ...
  },
  {
    title: "Easy Ordering",
    description:
      "No complex checkout! Simply inquire, confirm, and receive your products hassle-free.",
  },
];
  ...
  useEffect(() => {
    const fetchProducts = async (): Promise<void> => {
      try {
        ...
        setFeaturedProducts(
          featured.length > 3 ? featured.slice(0, 3) : featured
        );
      } catch (error) {
      ...
      try {
        ...
        if (filters.category || filters.ageGroup) {
          ...
          if (response.status === 404) {
            console.warn(
              "Filter endpoint not found, falling back to all products"
            );
  ...
  return (
    <>
      ...
      <section className="bg-white py-8 md:py-16 px-4 sm:px-6 lg:px-26 text-center">
        ...
        <div className="hidden lg:block mt-10 md:mt-20">
          <DottedPattern />
        </div>
            ...
            <div className="grid grid-cols-1 min-[500px]:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mt-8 sm:mt-10 px-2 sm:px-0">
              {displayedProducts &&
                displayedProducts.map((product) => (
                  <div
                    key={product._id}
                    className="bg-white shadow-lg w-full max-w-[410px] mx-auto sm:w-full rounded-2xl flex flex-col h-full"
                  >
          ...
          <div className="grid grid-cols-1 min-[500px]:grid-cols-2 min-[900px]:grid-cols-3 gap-4 min-[768px]:gap-6 lg:gap-8 mt-6 min-[768px]:mt-8 lg:mt-10 justify-items-center">
            {featuredProducts &&
              featuredProducts.map((product) => (
                <div
                  key={product._id}
                  className="bg-white shadow-lg w-full max-w-[400px] min-[900px]:max-w-[410px] rounded-2xl flex flex-col h-full overflow-hidden"
                >
      ...
      <section className="relative mt-12 lg:mt-23 bg-white py-8 lg:py-16 px-4 sm:px-8 md:px-20 lg:px-32 flex flex-col lg:flex-row items-center lg:items-start gap-6 lg:gap-12">
        ...
        <div className="mt-0 lg:mt-9 w-full lg:w-auto">
          ...
          <p className="w-full lg:w-auto text-lg md:text-xl lg:text-[26px] xl:text-[30px] font-normal leading-relaxed lg:leading-[44px] tracking-wide lg:tracking-[0.64px] text-[#456696] font-urbanist mt-4 lg:mt-7 text-center lg:text-left">
            Support your child's growth with expert-approved{" "}
            <br className="hidden lg:inline" /> therapy essentials!
          </p>
        </div>
                  ...
                  <div className="relative mb-4 mt-2">
                    ...
                    <div className="absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center">
                      <svg
                        ...
                        viewBox="0 0 60 60"
                        fill="none"
                        className="w-8 h-8 min-[768px]:w-[50px] min-[768px]:h-[50px] xl:w-[60px] xl:h-[60px]"
                      >
                        <path
